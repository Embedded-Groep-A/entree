  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (knop_triggered || alarm_triggered){
	      ...
	      HAL_Delay(2000); // kleine delay zodat de deur even open blijft staan
	      knop_triggered = 0;
	      alarm_triggered = 0;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        if (uart_rx_char == '?') {
            alarm_triggered = 1;  // trigger alarm
        }

        // Herstart de interrupt voor volgende byte
        //HAL_UART_Transmit_IT(&huart2,&uart_rx_char,sizeof(uart_rx_char),100);
        HAL_UART_Receive_IT(&huart2, &uart_rx_char, 1);
    }
